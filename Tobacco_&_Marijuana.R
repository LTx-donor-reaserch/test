...
# category
#==================================================
LTx$classification3 <- with(LTx, ifelse(Cig_N_F_C==0&m.history==0,0,
                                        ifelse(Cig_N_F_C==0 & m.history==1|Cig_N_F_C==1 & m.history==0|Cig_N_F_C==1 & m.history==1,1,
                                               ifelse(Cig_N_F_C==2|m.history==2,2,NA))))
...
ggsurv <- ggsurvplot(sf_graft_3y, break.time.by=1,title = "",legend.title = "",legend=c(.8,.2),legend.labs=c("NS donor","FS donor","CS donor"),font.legend=list(size = 17, color = "black"),
                     risk.table=T, size = 1.7,palette=c("lightcoral" ,"green3" ,"cornflowerblue"),linetype = c(1,4,3),risk.table.y.text = T,risk.table.size=5,
                     xlab="Time (years)",xlim=c(0,3),ylim=c(0,1),fontsize = 6)
ggpar(ggsurv,
      font.tickslab = 17,
      font.x        = 17,
      font.y        = 17)
...
COX <- coxph(Surv(surv3y,graft_3y)~current_cig+current_m+GIT.60 + Dx_CLAD + SLT, data=LTx)
summary(COX)

LTxx <- LTx %>% filter(Cig_N_F_C==0|!is.na(p.y.n))
COX <- coxph(Surv(surv3y,graft_3y)~smoking_10py+m.5y.weekly+GIT.60 + Dx_PH_CLAD + SLT, data=LTxx)
summary(COX)

# sensitivity analysis
## donor
COX <- coxph(Surv(surv3y,graft_3y)~current_cig+current_m+Age_more_55 + GIT+PF_300_CoLog, data=LTx)
summary(COX)

## Recipient
COX <- coxph(Surv(surv3y,graft_3y)~current_cig+current_m+Dx_CLAD + SLT+CPB, data=LTx)
summary(COX)
...
print.ci.summary(ci=ci, res=res)
...
summary.ci(ci = ci, res = res, event = 1)
